...
for sample in sample_list:
    L = df['dupcode'] == sample
    temp = df[L]
    df.loc[df['dupcode']==sample, 'difference'] = np.abs(np.diff(temp['total_nitrate_nitrite']))[0]
    df.loc[df['dupcode']==sample, 'mean'] = temp['total_nitrate_nitrite'].mean()
